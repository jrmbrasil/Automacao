;  ASM code generated by mikroVirtualMachine for PIC - V. 8.1.0.0
;  Date/Time: 08/06/2010 15:28:50
;  Info: http://www.mikroe.com


; ADDRESS	OPCODE	ASM
; ----------------------------------------------
$0000	$29A5			GOTO	_main
$035E	$	_Delay_1us:
;delays.c,7 :: 		void Delay_1us() {
;delays.c,8 :: 		Delay_us(1);
$035E	$3001			MOVLW	1
$035F	$1303			BCF	STATUS, RP1
$0360	$1283			BCF	STATUS, RP0
$0361	$00FA			MOVWF	STACK_10
$0362	$0BFA			DECFSZ	STACK_10, F
$0363	$2B62			GOTO	$-1
$0364	$0000			NOP
;delays.c,9 :: 		}
$0365	$0008			RETURN
$0344	$	_Delay_5500us:
;delays.c,31 :: 		void Delay_5500us() {
;delays.c,32 :: 		Delay_us(5500);
$0344	$3024			MOVLW	36
$0345	$1303			BCF	STATUS, RP1
$0346	$1283			BCF	STATUS, RP0
$0347	$00FB			MOVWF	STACK_11
$0348	$30FF			MOVLW	255
$0349	$00FA			MOVWF	STACK_10
$034A	$0BFB			DECFSZ	STACK_11, F
$034B	$2B4D			GOTO	$+2
$034C	$2B50			GOTO	$+4
$034D	$0BFA			DECFSZ	STACK_10, F
$034E	$2B4D			GOTO	$-1
$034F	$2B4A			GOTO	$-5
$0350	$309E			MOVLW	158
$0351	$00FA			MOVWF	STACK_10
$0352	$0BFA			DECFSZ	STACK_10, F
$0353	$2B52			GOTO	$-1
;delays.c,33 :: 		}
$0354	$0008			RETURN
$036E	$	_Delay_50us:
;delays.c,19 :: 		void Delay_50us() {
;delays.c,20 :: 		Delay_us(50);
$036E	$3053			MOVLW	83
$036F	$1303			BCF	STATUS, RP1
$0370	$1283			BCF	STATUS, RP0
$0371	$00FA			MOVWF	STACK_10
$0372	$0BFA			DECFSZ	STACK_10, F
$0373	$2B72			GOTO	$-1
;delays.c,21 :: 		}
$0374	$0008			RETURN
$0327	$	_Div_16x16_U:
$0327	$1303			BCF	STATUS, RP1
$0328	$1283			BCF	STATUS, RP0
$0329	$01F8			CLRF	STACK_8
$032A	$01F9			CLRF	STACK_9
$032B	$3010			MOVLW	16
$032C	$00FC			MOVWF	STACK_12
$032D	$0D71			RLF	STACK_1, W
$032E	$0DF8			RLF	STACK_8, F
$032F	$0DF9			RLF	STACK_9, F
$0330	$0874			MOVF	STACK_4, W
$0331	$02F8			SUBWF	STACK_8, F
$0332	$0875			MOVF	STACK_5, W
$0333	$1C03			BTFSS	STATUS, C
$0334	$0F75			INCFSZ	STACK_5, W
$0335	$02F9			SUBWF	STACK_9, F
$0336	$1803			BTFSC	STATUS, C
$0337	$2B3F			GOTO	$+8
$0338	$0874			MOVF	STACK_4, W
$0339	$07F8			ADDWF	STACK_8, F
$033A	$0875			MOVF	STACK_5, W
$033B	$1803			BTFSC	STATUS, C
$033C	$0F75			INCFSZ	STACK_5, W
$033D	$07F9			ADDWF	STACK_9, F
$033E	$1003			BCF	STATUS, C
$033F	$0DF0			RLF	STACK_0, F
$0340	$0DF1			RLF	STACK_1, F
$0341	$0BFC			DECFSZ	STACK_12, F
$0342	$2B2D			GOTO	$-21
$0343	$0008			RETURN
$0130	$	_Lcd_Custom_Cmd:
$0130	$1303			BCF	STATUS, RP1
$0131	$1283			BCF	STATUS, RP0
$0132	$0820			MOVF	Lcd_Custom_ctrlport_address, 0
$0133	$0084			MOVWF	FSR
$0134	$0921			COMF	Lcd_Custom____mENABLE, W
$0135	$00F0			MOVWF	STACK_0
$0136	$0870			MOVF	STACK_0, 0
$0137	$0580			ANDWF	INDF, 1
$0138	$0922			COMF	Lcd_Custom____mRS, W
$0139	$00F0			MOVWF	STACK_0
$013A	$0870			MOVF	STACK_0, 0
$013B	$0580			ANDWF	INDF, 1
$013C	$0823			MOVF	Lcd_Custom_dataport_address, 0
$013D	$0084			MOVWF	FSR
$013E	$0825			MOVF	Lcd_Custom____mDB6, 0
$013F	$0424			IORWF	Lcd_Custom____mDB7, 0
$0140	$00F0			MOVWF	STACK_0
$0141	$0826			MOVF	Lcd_Custom____mDB5, 0
$0142	$04F0			IORWF	STACK_0, 1
$0143	$0827			MOVF	Lcd_Custom____mDB4, 0
$0144	$04F0			IORWF	STACK_0, 1
$0145	$09F0			COMF	STACK_0, F
$0146	$0870			MOVF	STACK_0, 0
$0147	$0580			ANDWF	INDF, 1
$0148	$1FC4			BTFSS	FARG_Lcd_Custom_Cmd+0, 7
$0149	$294C			GOTO	L_Lcd_Custom_Cmd_0
$014A	$0824			MOVF	Lcd_Custom____mDB7, 0
$014B	$0480			IORWF	INDF, 1
$014C	$	L_Lcd_Custom_Cmd_0:
$014C	$1F44			BTFSS	FARG_Lcd_Custom_Cmd+0, 6
$014D	$2950			GOTO	L_Lcd_Custom_Cmd_1
$014E	$0825			MOVF	Lcd_Custom____mDB6, 0
$014F	$0480			IORWF	INDF, 1
$0150	$	L_Lcd_Custom_Cmd_1:
$0150	$1EC4			BTFSS	FARG_Lcd_Custom_Cmd+0, 5
$0151	$2954			GOTO	L_Lcd_Custom_Cmd_2
$0152	$0826			MOVF	Lcd_Custom____mDB5, 0
$0153	$0480			IORWF	INDF, 1
$0154	$	L_Lcd_Custom_Cmd_2:
$0154	$1E44			BTFSS	FARG_Lcd_Custom_Cmd+0, 4
$0155	$2958			GOTO	L_Lcd_Custom_Cmd_3
$0156	$0827			MOVF	Lcd_Custom____mDB4, 0
$0157	$0480			IORWF	INDF, 1
$0158	$	L_Lcd_Custom_Cmd_3:
$0158	$0820			MOVF	Lcd_Custom_ctrlport_address, 0
$0159	$0084			MOVWF	FSR
$015A	$0828			MOVF	Lcd_Custom_lcdcmd_status, 0
$015B	$1903			BTFSC	STATUS, Z
$015C	$2962			GOTO	L_Lcd_Custom_Cmd_4
$015D	$0922			COMF	Lcd_Custom____mRS, W
$015E	$00F0			MOVWF	STACK_0
$015F	$0870			MOVF	STACK_0, 0
$0160	$0580			ANDWF	INDF, 1
$0161	$2964			GOTO	L_Lcd_Custom_Cmd_5
$0162	$	L_Lcd_Custom_Cmd_4:
$0162	$0822			MOVF	Lcd_Custom____mRS, 0
$0163	$0480			IORWF	INDF, 1
$0164	$	L_Lcd_Custom_Cmd_5:
$0164	$0821			MOVF	Lcd_Custom____mENABLE, 0
$0165	$0480			IORWF	INDF, 1
$0166	$235E			CALL	_Delay_1us
$0167	$0921			COMF	Lcd_Custom____mENABLE, W
$0168	$00F0			MOVWF	STACK_0
$0169	$0870			MOVF	STACK_0, 0
$016A	$0580			ANDWF	INDF, 1
$016B	$0922			COMF	Lcd_Custom____mRS, W
$016C	$00F0			MOVWF	STACK_0
$016D	$0870			MOVF	STACK_0, 0
$016E	$0580			ANDWF	INDF, 1
$016F	$0823			MOVF	Lcd_Custom_dataport_address, 0
$0170	$0084			MOVWF	FSR
$0171	$0825			MOVF	Lcd_Custom____mDB6, 0
$0172	$0424			IORWF	Lcd_Custom____mDB7, 0
$0173	$00F0			MOVWF	STACK_0
$0174	$0826			MOVF	Lcd_Custom____mDB5, 0
$0175	$04F0			IORWF	STACK_0, 1
$0176	$0827			MOVF	Lcd_Custom____mDB4, 0
$0177	$04F0			IORWF	STACK_0, 1
$0178	$09F0			COMF	STACK_0, F
$0179	$0870			MOVF	STACK_0, 0
$017A	$0580			ANDWF	INDF, 1
$017B	$1DC4			BTFSS	FARG_Lcd_Custom_Cmd+0, 3
$017C	$297F			GOTO	L_Lcd_Custom_Cmd_6
$017D	$0824			MOVF	Lcd_Custom____mDB7, 0
$017E	$0480			IORWF	INDF, 1
$017F	$	L_Lcd_Custom_Cmd_6:
$017F	$1D44			BTFSS	FARG_Lcd_Custom_Cmd+0, 2
$0180	$2983			GOTO	L_Lcd_Custom_Cmd_7
$0181	$0825			MOVF	Lcd_Custom____mDB6, 0
$0182	$0480			IORWF	INDF, 1
$0183	$	L_Lcd_Custom_Cmd_7:
$0183	$1CC4			BTFSS	FARG_Lcd_Custom_Cmd+0, 1
$0184	$2987			GOTO	L_Lcd_Custom_Cmd_8
$0185	$0826			MOVF	Lcd_Custom____mDB5, 0
$0186	$0480			IORWF	INDF, 1
$0187	$	L_Lcd_Custom_Cmd_8:
$0187	$1C44			BTFSS	FARG_Lcd_Custom_Cmd+0, 0
$0188	$298B			GOTO	L_Lcd_Custom_Cmd_9
$0189	$0827			MOVF	Lcd_Custom____mDB4, 0
$018A	$0480			IORWF	INDF, 1
$018B	$	L_Lcd_Custom_Cmd_9:
$018B	$0820			MOVF	Lcd_Custom_ctrlport_address, 0
$018C	$0084			MOVWF	FSR
$018D	$0828			MOVF	Lcd_Custom_lcdcmd_status, 0
$018E	$1903			BTFSC	STATUS, Z
$018F	$2995			GOTO	L_Lcd_Custom_Cmd_10
$0190	$0922			COMF	Lcd_Custom____mRS, W
$0191	$00F0			MOVWF	STACK_0
$0192	$0870			MOVF	STACK_0, 0
$0193	$0580			ANDWF	INDF, 1
$0194	$2997			GOTO	L_Lcd_Custom_Cmd_11
$0195	$	L_Lcd_Custom_Cmd_10:
$0195	$0822			MOVF	Lcd_Custom____mRS, 0
$0196	$0480			IORWF	INDF, 1
$0197	$	L_Lcd_Custom_Cmd_11:
$0197	$0821			MOVF	Lcd_Custom____mENABLE, 0
$0198	$0480			IORWF	INDF, 1
$0199	$235E			CALL	_Delay_1us
$019A	$0921			COMF	Lcd_Custom____mENABLE, W
$019B	$00F0			MOVWF	STACK_0
$019C	$0870			MOVF	STACK_0, 0
$019D	$0580			ANDWF	INDF, 1
$019E	$0828			MOVF	Lcd_Custom_lcdcmd_status, 0
$019F	$1903			BTFSC	STATUS, Z
$01A0	$29A3			GOTO	L_Lcd_Custom_Cmd_12
$01A1	$2344			CALL	_Delay_5500us
$01A2	$29A4			GOTO	L_Lcd_Custom_Cmd_13
$01A3	$	L_Lcd_Custom_Cmd_12:
$01A3	$236E			CALL	_Delay_50us
$01A4	$	L_Lcd_Custom_Cmd_13:
$01A4	$0008			RETURN
$0366	$	_Delay_22us:
;delays.c,15 :: 		void Delay_22us() {
;delays.c,16 :: 		Delay_us(22);
$0366	$3024			MOVLW	36
$0367	$1303			BCF	STATUS, RP1
$0368	$1283			BCF	STATUS, RP0
$0369	$00FA			MOVWF	STACK_10
$036A	$0BFA			DECFSZ	STACK_10, F
$036B	$2B6A			GOTO	$-1
$036C	$0000			NOP
;delays.c,17 :: 		}
$036D	$0008			RETURN
$0209	$	_WordToStr:
$0209	$1303			BCF	STATUS, RP1
$020A	$1283			BCF	STATUS, RP0
$020B	$0841			MOVF	FARG_WordToStr+2, 0
$020C	$0084			MOVWF	FSR
$020D	$3020			MOVLW	32
$020E	$0080			MOVWF	INDF
$020F	$0A41			INCF	FARG_WordToStr+2, 0
$0210	$00F0			MOVWF	STACK_0
$0211	$0870			MOVF	STACK_0, 0
$0212	$0084			MOVWF	FSR
$0213	$3020			MOVLW	32
$0214	$0080			MOVWF	INDF
$0215	$3002			MOVLW	2
$0216	$0741			ADDWF	FARG_WordToStr+2, 0
$0217	$00F0			MOVWF	STACK_0
$0218	$0870			MOVF	STACK_0, 0
$0219	$0084			MOVWF	FSR
$021A	$3020			MOVLW	32
$021B	$0080			MOVWF	INDF
$021C	$3003			MOVLW	3
$021D	$0741			ADDWF	FARG_WordToStr+2, 0
$021E	$00F0			MOVWF	STACK_0
$021F	$0870			MOVF	STACK_0, 0
$0220	$0084			MOVWF	FSR
$0221	$3020			MOVLW	32
$0222	$0080			MOVWF	INDF
$0223	$3004			MOVLW	4
$0224	$0741			ADDWF	FARG_WordToStr+2, 0
$0225	$00F0			MOVWF	STACK_0
$0226	$0870			MOVF	STACK_0, 0
$0227	$0084			MOVWF	FSR
$0228	$3030			MOVLW	48
$0229	$0080			MOVWF	INDF
$022A	$3005			MOVLW	5
$022B	$0741			ADDWF	FARG_WordToStr+2, 0
$022C	$00F0			MOVWF	STACK_0
$022D	$0870			MOVF	STACK_0, 0
$022E	$0084			MOVWF	FSR
$022F	$0180			CLRF	INDF, 1
$0230	$3004			MOVLW	4
$0231	$00C2			MOVWF	WordToStr_pos_L0
$0232	$	L_WordToStr_4:
$0232	$0840			MOVF	FARG_WordToStr+1, 0
$0233	$3C00			SUBLW	0
$0234	$1D03			BTFSS	STATUS, Z
$0235	$2A38			GOTO	L_WordToStr_51
$0236	$083F			MOVF	FARG_WordToStr+0, 0
$0237	$3C00			SUBLW	0
$0238	$	L_WordToStr_51:
$0238	$1803			BTFSC	STATUS, C
$0239	$2A5D			GOTO	L_WordToStr_5
$023A	$0842			MOVF	WordToStr_pos_L0, 0
$023B	$0741			ADDWF	FARG_WordToStr+2, 0
$023C	$00C3			MOVWF	FLOC_WordToStr+1
$023D	$300A			MOVLW	10
$023E	$00F4			MOVWF	STACK_4
$023F	$01F5			CLRF	STACK_4+1
$0240	$083F			MOVF	FARG_WordToStr+0, 0
$0241	$00F0			MOVWF	STACK_0
$0242	$0840			MOVF	FARG_WordToStr+1, 0
$0243	$00F1			MOVWF	STACK_0+1
$0244	$2327			CALL	_div_16x16_u
$0245	$0878			MOVF	STACK_8, 0
$0246	$00F0			MOVWF	STACK_0
$0247	$0879			MOVF	STACK_9, 0
$0248	$00F1			MOVWF	STACK_1
$0249	$3030			MOVLW	48
$024A	$07F0			ADDWF	STACK_0, 1
$024B	$0843			MOVF	FLOC_WordToStr+1, 0
$024C	$0084			MOVWF	FSR
$024D	$0870			MOVF	STACK_0, 0
$024E	$0080			MOVWF	INDF
$024F	$300A			MOVLW	10
$0250	$00F4			MOVWF	STACK_4
$0251	$01F5			CLRF	STACK_4+1
$0252	$083F			MOVF	FARG_WordToStr+0, 0
$0253	$00F0			MOVWF	STACK_0
$0254	$0840			MOVF	FARG_WordToStr+1, 0
$0255	$00F1			MOVWF	STACK_0+1
$0256	$2327			CALL	_div_16x16_u
$0257	$0870			MOVF	STACK_0, 0
$0258	$00BF			MOVWF	FARG_WordToStr+0
$0259	$0871			MOVF	STACK_0+1, 0
$025A	$00C0			MOVWF	FARG_WordToStr+1
$025B	$03C2			DECF	WordToStr_pos_L0, 1
$025C	$2A32			GOTO	L_WordToStr_4
$025D	$	L_WordToStr_5:
$025D	$0008			RETURN
$0355	$	_Lcd_Custom_Chr_CP:
$0355	$1303			BCF	STATUS, RP1
$0356	$1283			BCF	STATUS, RP0
$0357	$01A8			CLRF	Lcd_Custom_lcdcmd_status, 1
$0358	$083F			MOVF	FARG_Lcd_Custom_Chr_CP+0, 0
$0359	$00C4			MOVWF	FARG_Lcd_Custom_Cmd+0
$035A	$2130			CALL	_Lcd_Custom_Cmd
$035B	$3001			MOVLW	1
$035C	$00A8			MOVWF	Lcd_Custom_lcdcmd_status
$035D	$0008			RETURN
$0004	$	_Lcd_Custom_Config:
$0004	$2344			CALL	_Delay_5500us
$0005	$2344			CALL	_Delay_5500us
$0006	$2344			CALL	_Delay_5500us
$0007	$0837			MOVF	FARG_Lcd_Custom_Config+1, 0
$0008	$00F0			MOVWF	STACK_0
$0009	$3001			MOVLW	1
$000A	$00F7			MOVWF	STACK_7
$000B	$0870			MOVF	STACK_0, 0
$000C	$	L_Lcd_Custom_Config_32:
$000C	$1903			BTFSC	STATUS, Z
$000D	$2812			GOTO	L_Lcd_Custom_Config_33
$000E	$0DF7			RLF	STACK_7, 1
$000F	$1077			BCF	STACK_7, 0
$0010	$3FFF			ADDLW	255
$0011	$280C			GOTO	L_Lcd_Custom_Config_32
$0012	$	L_Lcd_Custom_Config_33:
$0012	$0877			MOVF	STACK_7, 0
$0013	$00A4			MOVWF	Lcd_Custom____mDB7
$0014	$0838			MOVF	FARG_Lcd_Custom_Config+2, 0
$0015	$00F0			MOVWF	STACK_0
$0016	$3001			MOVLW	1
$0017	$00F1			MOVWF	STACK_1
$0018	$0870			MOVF	STACK_0, 0
$0019	$	L_Lcd_Custom_Config_34:
$0019	$1903			BTFSC	STATUS, Z
$001A	$281F			GOTO	L_Lcd_Custom_Config_35
$001B	$0DF1			RLF	STACK_1, 1
$001C	$1071			BCF	STACK_1, 0
$001D	$3FFF			ADDLW	255
$001E	$2819			GOTO	L_Lcd_Custom_Config_34
$001F	$	L_Lcd_Custom_Config_35:
$001F	$0871			MOVF	STACK_1, 0
$0020	$00A5			MOVWF	Lcd_Custom____mDB6
$0021	$0839			MOVF	FARG_Lcd_Custom_Config+3, 0
$0022	$00F0			MOVWF	STACK_0
$0023	$3001			MOVLW	1
$0024	$00F6			MOVWF	STACK_6
$0025	$0870			MOVF	STACK_0, 0
$0026	$	L_Lcd_Custom_Config_36:
$0026	$1903			BTFSC	STATUS, Z
$0027	$282C			GOTO	L_Lcd_Custom_Config_37
$0028	$0DF6			RLF	STACK_6, 1
$0029	$1076			BCF	STACK_6, 0
$002A	$3FFF			ADDLW	255
$002B	$2826			GOTO	L_Lcd_Custom_Config_36
$002C	$	L_Lcd_Custom_Config_37:
$002C	$0876			MOVF	STACK_6, 0
$002D	$00A6			MOVWF	Lcd_Custom____mDB5
$002E	$083A			MOVF	FARG_Lcd_Custom_Config+4, 0
$002F	$00F0			MOVWF	STACK_0
$0030	$3001			MOVLW	1
$0031	$00F5			MOVWF	STACK_5
$0032	$0870			MOVF	STACK_0, 0
$0033	$	L_Lcd_Custom_Config_38:
$0033	$1903			BTFSC	STATUS, Z
$0034	$2839			GOTO	L_Lcd_Custom_Config_39
$0035	$0DF5			RLF	STACK_5, 1
$0036	$1075			BCF	STACK_5, 0
$0037	$3FFF			ADDLW	255
$0038	$2833			GOTO	L_Lcd_Custom_Config_38
$0039	$	L_Lcd_Custom_Config_39:
$0039	$0875			MOVF	STACK_5, 0
$003A	$00A7			MOVWF	Lcd_Custom____mDB4
$003B	$083C			MOVF	FARG_Lcd_Custom_Config+6, 0
$003C	$00F0			MOVWF	STACK_0
$003D	$3001			MOVLW	1
$003E	$00F4			MOVWF	STACK_4
$003F	$0870			MOVF	STACK_0, 0
$0040	$	L_Lcd_Custom_Config_40:
$0040	$1903			BTFSC	STATUS, Z
$0041	$2846			GOTO	L_Lcd_Custom_Config_41
$0042	$0DF4			RLF	STACK_4, 1
$0043	$1074			BCF	STACK_4, 0
$0044	$3FFF			ADDLW	255
$0045	$2840			GOTO	L_Lcd_Custom_Config_40
$0046	$	L_Lcd_Custom_Config_41:
$0046	$0874			MOVF	STACK_4, 0
$0047	$00A2			MOVWF	Lcd_Custom____mRS
$0048	$083D			MOVF	FARG_Lcd_Custom_Config+7, 0
$0049	$00F0			MOVWF	STACK_0
$004A	$3001			MOVLW	1
$004B	$00F3			MOVWF	STACK_3
$004C	$0870			MOVF	STACK_0, 0
$004D	$	L_Lcd_Custom_Config_42:
$004D	$1903			BTFSC	STATUS, Z
$004E	$2853			GOTO	L_Lcd_Custom_Config_43
$004F	$0DF3			RLF	STACK_3, 1
$0050	$1073			BCF	STACK_3, 0
$0051	$3FFF			ADDLW	255
$0052	$284D			GOTO	L_Lcd_Custom_Config_42
$0053	$	L_Lcd_Custom_Config_43:
$0053	$0873			MOVF	STACK_3, 0
$0054	$00A9			MOVWF	Lcd_Custom____mRW
$0055	$083E			MOVF	FARG_Lcd_Custom_Config+8, 0
$0056	$00F0			MOVWF	STACK_0
$0057	$3001			MOVLW	1
$0058	$00F2			MOVWF	STACK_2
$0059	$0870			MOVF	STACK_0, 0
$005A	$	L_Lcd_Custom_Config_44:
$005A	$1903			BTFSC	STATUS, Z
$005B	$2860			GOTO	L_Lcd_Custom_Config_45
$005C	$0DF2			RLF	STACK_2, 1
$005D	$1072			BCF	STACK_2, 0
$005E	$3FFF			ADDLW	255
$005F	$285A			GOTO	L_Lcd_Custom_Config_44
$0060	$	L_Lcd_Custom_Config_45:
$0060	$0872			MOVF	STACK_2, 0
$0061	$00A1			MOVWF	Lcd_Custom____mENABLE
$0062	$0836			MOVF	FARG_Lcd_Custom_Config+0, 0
$0063	$00A3			MOVWF	Lcd_Custom_dataport_address
$0064	$083B			MOVF	FARG_Lcd_Custom_Config+5, 0
$0065	$00A0			MOVWF	Lcd_Custom_ctrlport_address
$0066	$0836			MOVF	FARG_Lcd_Custom_Config+0, 0
$0067	$0084			MOVWF	FSR
$0068	$0871			MOVF	STACK_1, 0
$0069	$0477			IORWF	STACK_7, 0
$006A	$00F0			MOVWF	STACK_0
$006B	$0876			MOVF	STACK_6, 0
$006C	$04F0			IORWF	STACK_0, 1
$006D	$0875			MOVF	STACK_5, 0
$006E	$0470			IORWF	STACK_0, 0
$006F	$00F1			MOVWF	STACK_1
$0070	$0971			COMF	STACK_1, W
$0071	$00F0			MOVWF	STACK_0
$0072	$0870			MOVF	STACK_0, 0
$0073	$0580			ANDWF	INDF, 1
$0074	$1784			BSF	FSR, 7
$0075	$0871			MOVF	STACK_1, 0
$0076	$00F0			MOVWF	STACK_0
$0077	$09F0			COMF	STACK_0, F
$0078	$0870			MOVF	STACK_0, 0
$0079	$0580			ANDWF	INDF, 1
$007A	$083B			MOVF	FARG_Lcd_Custom_Config+5, 0
$007B	$0084			MOVWF	FSR
$007C	$0873			MOVF	STACK_3, 0
$007D	$0474			IORWF	STACK_4, 0
$007E	$00F0			MOVWF	STACK_0
$007F	$0872			MOVF	STACK_2, 0
$0080	$04F0			IORWF	STACK_0, 1
$0081	$09F0			COMF	STACK_0, F
$0082	$0870			MOVF	STACK_0, 0
$0083	$0580			ANDWF	INDF, 1
$0084	$1784			BSF	FSR, 7
$0085	$0870			MOVF	STACK_0, 0
$0086	$0580			ANDWF	INDF, 1
$0087	$0836			MOVF	FARG_Lcd_Custom_Config+0, 0
$0088	$0084			MOVWF	FSR
$0089	$0875			MOVF	STACK_5, 0
$008A	$0476			IORWF	STACK_6, 0
$008B	$00F0			MOVWF	STACK_0
$008C	$0870			MOVF	STACK_0, 0
$008D	$0480			IORWF	INDF, 1
$008E	$083B			MOVF	FARG_Lcd_Custom_Config+5, 0
$008F	$0084			MOVWF	FSR
$0090	$0872			MOVF	STACK_2, 0
$0091	$0480			IORWF	INDF, 1
$0092	$235E			CALL	_Delay_1us
$0093	$0921			COMF	Lcd_Custom____mENABLE, W
$0094	$00F0			MOVWF	STACK_0
$0095	$0870			MOVF	STACK_0, 0
$0096	$0580			ANDWF	INDF, 1
$0097	$2344			CALL	_Delay_5500us
$0098	$0821			MOVF	Lcd_Custom____mENABLE, 0
$0099	$0480			IORWF	INDF, 1
$009A	$235E			CALL	_Delay_1us
$009B	$0921			COMF	Lcd_Custom____mENABLE, W
$009C	$00F0			MOVWF	STACK_0
$009D	$0870			MOVF	STACK_0, 0
$009E	$0580			ANDWF	INDF, 1
$009F	$2344			CALL	_Delay_5500us
$00A0	$0821			MOVF	Lcd_Custom____mENABLE, 0
$00A1	$0480			IORWF	INDF, 1
$00A2	$235E			CALL	_Delay_1us
$00A3	$0921			COMF	Lcd_Custom____mENABLE, W
$00A4	$00F0			MOVWF	STACK_0
$00A5	$0870			MOVF	STACK_0, 0
$00A6	$0580			ANDWF	INDF, 1
$00A7	$2344			CALL	_Delay_5500us
$00A8	$0823			MOVF	Lcd_Custom_dataport_address, 0
$00A9	$0084			MOVWF	FSR
$00AA	$0826			MOVF	Lcd_Custom____mDB5, 0
$00AB	$0480			IORWF	INDF, 1
$00AC	$0927			COMF	Lcd_Custom____mDB4, W
$00AD	$00F0			MOVWF	STACK_0
$00AE	$0870			MOVF	STACK_0, 0
$00AF	$0580			ANDWF	INDF, 1
$00B0	$0820			MOVF	Lcd_Custom_ctrlport_address, 0
$00B1	$0084			MOVWF	FSR
$00B2	$0821			MOVF	Lcd_Custom____mENABLE, 0
$00B3	$0480			IORWF	INDF, 1
$00B4	$235E			CALL	_Delay_1us
$00B5	$0921			COMF	Lcd_Custom____mENABLE, W
$00B6	$00F0			MOVWF	STACK_0
$00B7	$0870			MOVF	STACK_0, 0
$00B8	$0580			ANDWF	INDF, 1
$00B9	$2344			CALL	_Delay_5500us
$00BA	$0821			MOVF	Lcd_Custom____mENABLE, 0
$00BB	$0480			IORWF	INDF, 1
$00BC	$235E			CALL	_Delay_1us
$00BD	$0921			COMF	Lcd_Custom____mENABLE, W
$00BE	$00F0			MOVWF	STACK_0
$00BF	$0870			MOVF	STACK_0, 0
$00C0	$0580			ANDWF	INDF, 1
$00C1	$0823			MOVF	Lcd_Custom_dataport_address, 0
$00C2	$0084			MOVWF	FSR
$00C3	$0926			COMF	Lcd_Custom____mDB5, W
$00C4	$00F0			MOVWF	STACK_0
$00C5	$0870			MOVF	STACK_0, 0
$00C6	$0580			ANDWF	INDF, 1
$00C7	$0824			MOVF	Lcd_Custom____mDB7, 0
$00C8	$0480			IORWF	INDF, 1
$00C9	$0820			MOVF	Lcd_Custom_ctrlport_address, 0
$00CA	$0084			MOVWF	FSR
$00CB	$0821			MOVF	Lcd_Custom____mENABLE, 0
$00CC	$0480			IORWF	INDF, 1
$00CD	$235E			CALL	_Delay_1us
$00CE	$0921			COMF	Lcd_Custom____mENABLE, W
$00CF	$00F0			MOVWF	STACK_0
$00D0	$0870			MOVF	STACK_0, 0
$00D1	$0580			ANDWF	INDF, 1
$00D2	$2344			CALL	_Delay_5500us
$00D3	$0823			MOVF	Lcd_Custom_dataport_address, 0
$00D4	$0084			MOVWF	FSR
$00D5	$0924			COMF	Lcd_Custom____mDB7, W
$00D6	$00F0			MOVWF	STACK_0
$00D7	$0870			MOVF	STACK_0, 0
$00D8	$0580			ANDWF	INDF, 1
$00D9	$0827			MOVF	Lcd_Custom____mDB4, 0
$00DA	$0480			IORWF	INDF, 1
$00DB	$0820			MOVF	Lcd_Custom_ctrlport_address, 0
$00DC	$0084			MOVWF	FSR
$00DD	$0821			MOVF	Lcd_Custom____mENABLE, 0
$00DE	$0480			IORWF	INDF, 1
$00DF	$235E			CALL	_Delay_1us
$00E0	$0921			COMF	Lcd_Custom____mENABLE, W
$00E1	$00F0			MOVWF	STACK_0
$00E2	$0870			MOVF	STACK_0, 0
$00E3	$0580			ANDWF	INDF, 1
$00E4	$0823			MOVF	Lcd_Custom_dataport_address, 0
$00E5	$0084			MOVWF	FSR
$00E6	$0927			COMF	Lcd_Custom____mDB4, W
$00E7	$00F0			MOVWF	STACK_0
$00E8	$0870			MOVF	STACK_0, 0
$00E9	$0580			ANDWF	INDF, 1
$00EA	$0820			MOVF	Lcd_Custom_ctrlport_address, 0
$00EB	$0084			MOVWF	FSR
$00EC	$0821			MOVF	Lcd_Custom____mENABLE, 0
$00ED	$0480			IORWF	INDF, 1
$00EE	$235E			CALL	_Delay_1us
$00EF	$0921			COMF	Lcd_Custom____mENABLE, W
$00F0	$00F0			MOVWF	STACK_0
$00F1	$0870			MOVF	STACK_0, 0
$00F2	$0580			ANDWF	INDF, 1
$00F3	$2344			CALL	_Delay_5500us
$00F4	$0821			MOVF	Lcd_Custom____mENABLE, 0
$00F5	$0480			IORWF	INDF, 1
$00F6	$235E			CALL	_Delay_1us
$00F7	$0921			COMF	Lcd_Custom____mENABLE, W
$00F8	$00F0			MOVWF	STACK_0
$00F9	$0870			MOVF	STACK_0, 0
$00FA	$0580			ANDWF	INDF, 1
$00FB	$0823			MOVF	Lcd_Custom_dataport_address, 0
$00FC	$0084			MOVWF	FSR
$00FD	$0827			MOVF	Lcd_Custom____mDB4, 0
$00FE	$0480			IORWF	INDF, 1
$00FF	$0820			MOVF	Lcd_Custom_ctrlport_address, 0
$0100	$0084			MOVWF	FSR
$0101	$0821			MOVF	Lcd_Custom____mENABLE, 0
$0102	$0480			IORWF	INDF, 1
$0103	$235E			CALL	_Delay_1us
$0104	$0921			COMF	Lcd_Custom____mENABLE, W
$0105	$00F0			MOVWF	STACK_0
$0106	$0870			MOVF	STACK_0, 0
$0107	$0580			ANDWF	INDF, 1
$0108	$2344			CALL	_Delay_5500us
$0109	$0823			MOVF	Lcd_Custom_dataport_address, 0
$010A	$0084			MOVWF	FSR
$010B	$0927			COMF	Lcd_Custom____mDB4, W
$010C	$00F0			MOVWF	STACK_0
$010D	$0870			MOVF	STACK_0, 0
$010E	$0580			ANDWF	INDF, 1
$010F	$0820			MOVF	Lcd_Custom_ctrlport_address, 0
$0110	$0084			MOVWF	FSR
$0111	$0821			MOVF	Lcd_Custom____mENABLE, 0
$0112	$0480			IORWF	INDF, 1
$0113	$235E			CALL	_Delay_1us
$0114	$0921			COMF	Lcd_Custom____mENABLE, W
$0115	$00F0			MOVWF	STACK_0
$0116	$0870			MOVF	STACK_0, 0
$0117	$0580			ANDWF	INDF, 1
$0118	$0823			MOVF	Lcd_Custom_dataport_address, 0
$0119	$0084			MOVWF	FSR
$011A	$0826			MOVF	Lcd_Custom____mDB5, 0
$011B	$0427			IORWF	Lcd_Custom____mDB4, 0
$011C	$00F0			MOVWF	STACK_0
$011D	$0825			MOVF	Lcd_Custom____mDB6, 0
$011E	$04F0			IORWF	STACK_0, 1
$011F	$0824			MOVF	Lcd_Custom____mDB7, 0
$0120	$04F0			IORWF	STACK_0, 1
$0121	$0870			MOVF	STACK_0, 0
$0122	$0480			IORWF	INDF, 1
$0123	$0820			MOVF	Lcd_Custom_ctrlport_address, 0
$0124	$0084			MOVWF	FSR
$0125	$0821			MOVF	Lcd_Custom____mENABLE, 0
$0126	$0480			IORWF	INDF, 1
$0127	$235E			CALL	_Delay_1us
$0128	$0921			COMF	Lcd_Custom____mENABLE, W
$0129	$00F0			MOVWF	STACK_0
$012A	$0870			MOVF	STACK_0, 0
$012B	$0580			ANDWF	INDF, 1
$012C	$2344			CALL	_Delay_5500us
$012D	$3001			MOVLW	1
$012E	$00A8			MOVWF	Lcd_Custom_lcdcmd_status
$012F	$0008			RETURN
$02F9	$	_Adc_Read:
$02F9	$1303			BCF	STATUS, RP1
$02FA	$1283			BCF	STATUS, RP0
$02FB	$019F			CLRF	ADCON0, 1
$02FC	$30C0			MOVLW	192
$02FD	$049F			IORWF	ADCON0, 1
$02FE	$1683			BSF	STATUS, RP0
$02FF	$171F			BSF	ADCON1, 6
$0300	$179F			BSF	ADCON1, 7
$0301	$1283			BCF	STATUS, RP0
$0302	$0836			MOVF	FARG_Adc_Read+0, 0
$0303	$00F0			MOVWF	STACK_0
$0304	$0DF0			RLF	STACK_0, 1
$0305	$1070			BCF	STACK_0, 0
$0306	$0DF0			RLF	STACK_0, 1
$0307	$1070			BCF	STACK_0, 0
$0308	$0DF0			RLF	STACK_0, 1
$0309	$1070			BCF	STACK_0, 0
$030A	$0870			MOVF	STACK_0, 0
$030B	$049F			IORWF	ADCON0, 1
$030C	$141F			BSF	ADCON0, 0
$030D	$2366			CALL	_Delay_22us
$030E	$151F			BSF	ADCON0, 2
$030F	$	L_Adc_Read_0:
$030F	$1D1F			BTFSS	ADCON0, 2
$0310	$2B13			GOTO	L_Adc_Read_1
$0311	$0000			NOP
$0312	$2B0F			GOTO	L_Adc_Read_0
$0313	$	L_Adc_Read_1:
$0313	$101F			BCF	ADCON0, 0
$0314	$3008			MOVLW	8
$0315	$00F2			MOVWF	STACK_2
$0316	$081E			MOVF	ADRESH, 0
$0317	$00F0			MOVWF	STACK_0
$0318	$01F1			CLRF	STACK_0+1
$0319	$0872			MOVF	STACK_2, 0
$031A	$	L_Adc_Read_2:
$031A	$1903			BTFSC	STATUS, Z
$031B	$2B21			GOTO	L_Adc_Read_3
$031C	$0DF0			RLF	STACK_0, 1
$031D	$0DF1			RLF	STACK_0+1, 1
$031E	$1070			BCF	STACK_0, 0
$031F	$3FFF			ADDLW	255
$0320	$2B1A			GOTO	L_Adc_Read_2
$0321	$	L_Adc_Read_3:
$0321	$1683			BSF	STATUS, RP0
$0322	$081E			MOVF	ADRESL, 0
$0323	$04F0			IORWF	STACK_0, 1
$0324	$3000			MOVLW	0
$0325	$04F1			IORWF	STACK_0+1, 1
$0326	$0008			RETURN
$025E	$	_IntToStr:
$025E	$1303			BCF	STATUS, RP1
$025F	$1283			BCF	STATUS, RP0
$0260	$01BA			CLRF	IntToStr_negative_L0, 1
$0261	$0836			MOVF	FARG_IntToStr+0, 0
$0262	$00BB			MOVWF	IntToStr_inword_L0
$0263	$0837			MOVF	FARG_IntToStr+1, 0
$0264	$00BC			MOVWF	IntToStr_inword_L0+1
$0265	$3080			MOVLW	128
$0266	$0637			XORWF	FARG_IntToStr+1, 0
$0267	$00F0			MOVWF	STACK_0
$0268	$3080			MOVLW	128
$0269	$0270			SUBWF	STACK_0, 0
$026A	$1D03			BTFSS	STATUS, Z
$026B	$2A6E			GOTO	L_IntToStr_52
$026C	$3000			MOVLW	0
$026D	$0236			SUBWF	FARG_IntToStr+0, 0
$026E	$	L_IntToStr_52:
$026E	$1803			BTFSC	STATUS, C
$026F	$2A7A			GOTO	L_IntToStr_12
$0270	$3001			MOVLW	1
$0271	$00BA			MOVWF	IntToStr_negative_L0
$0272	$0836			MOVF	FARG_IntToStr+0, 0
$0273	$3C00			SUBLW	0
$0274	$00BB			MOVWF	IntToStr_inword_L0
$0275	$0837			MOVF	FARG_IntToStr+1, 0
$0276	$1C03			BTFSS	STATUS, C
$0277	$3F01			ADDLW	1
$0278	$01BC			CLRF	IntToStr_inword_L0+1
$0279	$02BC			SUBWF	IntToStr_inword_L0+1, 1
$027A	$	L_IntToStr_12:
$027A	$083B			MOVF	IntToStr_inword_L0, 0
$027B	$00BF			MOVWF	FARG_WordToStr+0
$027C	$083C			MOVF	IntToStr_inword_L0+1, 0
$027D	$00C0			MOVWF	FARG_WordToStr+1
$027E	$0838			MOVF	FARG_IntToStr+2, 0
$027F	$00C1			MOVWF	FARG_WordToStr+2
$0280	$2209			CALL	_WordToStr
$0281	$3006			MOVLW	6
$0282	$00B9			MOVWF	IntToStr_i_L0
$0283	$	L_IntToStr_13:
$0283	$0839			MOVF	IntToStr_i_L0, 0
$0284	$3C00			SUBLW	0
$0285	$1803			BTFSC	STATUS, C
$0286	$2A9B			GOTO	L_IntToStr_14
$0287	$0839			MOVF	IntToStr_i_L0, 0
$0288	$0738			ADDWF	FARG_IntToStr+2, 0
$0289	$00F2			MOVWF	STACK_2
$028A	$3001			MOVLW	1
$028B	$0239			SUBWF	IntToStr_i_L0, 0
$028C	$00F0			MOVWF	STACK_0
$028D	$01F1			CLRF	STACK_0+1
$028E	$1C03			BTFSS	STATUS, C
$028F	$03F1			DECF	STACK_0+1, 1
$0290	$0870			MOVF	STACK_0, 0
$0291	$0738			ADDWF	FARG_IntToStr+2, 0
$0292	$0084			MOVWF	FSR
$0293	$0800			MOVF	INDF, 0
$0294	$00F0			MOVWF	STACK_0
$0295	$0872			MOVF	STACK_2, 0
$0296	$0084			MOVWF	FSR
$0297	$0870			MOVF	STACK_0, 0
$0298	$0080			MOVWF	INDF
$0299	$03B9			DECF	IntToStr_i_L0, 1
$029A	$2A83			GOTO	L_IntToStr_13
$029B	$	L_IntToStr_14:
$029B	$0838			MOVF	FARG_IntToStr+2, 0
$029C	$0084			MOVWF	FSR
$029D	$3020			MOVLW	32
$029E	$0080			MOVWF	INDF
$029F	$083A			MOVF	IntToStr_negative_L0, 0
$02A0	$1903			BTFSC	STATUS, Z
$02A1	$2AB5			GOTO	L_IntToStr_15
$02A2	$	L_IntToStr_16:
$02A2	$0839			MOVF	IntToStr_i_L0, 0
$02A3	$0738			ADDWF	FARG_IntToStr+2, 0
$02A4	$0084			MOVWF	FSR
$02A5	$0800			MOVF	INDF, 0
$02A6	$00F2			MOVWF	STACK_2
$02A7	$0872			MOVF	STACK_2, 0
$02A8	$3A20			XORLW	32
$02A9	$1D03			BTFSS	STATUS, Z
$02AA	$2AAD			GOTO	L_IntToStr_17
$02AB	$0AB9			INCF	IntToStr_i_L0, 1
$02AC	$2AA2			GOTO	L_IntToStr_16
$02AD	$	L_IntToStr_17:
$02AD	$03B9			DECF	IntToStr_i_L0, 1
$02AE	$0839			MOVF	IntToStr_i_L0, 0
$02AF	$0738			ADDWF	FARG_IntToStr+2, 0
$02B0	$00F0			MOVWF	STACK_0
$02B1	$0870			MOVF	STACK_0, 0
$02B2	$0084			MOVWF	FSR
$02B3	$302D			MOVLW	45
$02B4	$0080			MOVWF	INDF
$02B5	$	L_IntToStr_15:
$02B5	$0008			RETURN
$02B6	$	_Lcd_Custom_Out:
$02B6	$2AC8			GOTO	L_Lcd_Custom_Out_23
$02B7	$	L_Lcd_Custom_Out_25:
$02B7	$3080			MOVLW	128
$02B8	$1303			BCF	STATUS, RP1
$02B9	$1283			BCF	STATUS, RP0
$02BA	$00B6			MOVWF	FARG_Lcd_Custom_Out+0
$02BB	$2ADB			GOTO	L_Lcd_Custom_Out_24
$02BC	$	L_Lcd_Custom_Out_26:
$02BC	$30C0			MOVLW	192
$02BD	$00B6			MOVWF	FARG_Lcd_Custom_Out+0
$02BE	$2ADB			GOTO	L_Lcd_Custom_Out_24
$02BF	$	L_Lcd_Custom_Out_27:
$02BF	$3094			MOVLW	148
$02C0	$00B6			MOVWF	FARG_Lcd_Custom_Out+0
$02C1	$2ADB			GOTO	L_Lcd_Custom_Out_24
$02C2	$	L_Lcd_Custom_Out_28:
$02C2	$30D4			MOVLW	212
$02C3	$00B6			MOVWF	FARG_Lcd_Custom_Out+0
$02C4	$2ADB			GOTO	L_Lcd_Custom_Out_24
$02C5	$	L_Lcd_Custom_Out_29:
$02C5	$3080			MOVLW	128
$02C6	$00B6			MOVWF	FARG_Lcd_Custom_Out+0
$02C7	$2ADB			GOTO	L_Lcd_Custom_Out_24
$02C8	$	L_Lcd_Custom_Out_23:
$02C8	$1303			BCF	STATUS, RP1
$02C9	$1283			BCF	STATUS, RP0
$02CA	$0836			MOVF	FARG_Lcd_Custom_Out+0, 0
$02CB	$3A01			XORLW	1
$02CC	$1903			BTFSC	STATUS, Z
$02CD	$2AB7			GOTO	L_Lcd_Custom_Out_25
$02CE	$0836			MOVF	FARG_Lcd_Custom_Out+0, 0
$02CF	$3A02			XORLW	2
$02D0	$1903			BTFSC	STATUS, Z
$02D1	$2ABC			GOTO	L_Lcd_Custom_Out_26
$02D2	$0836			MOVF	FARG_Lcd_Custom_Out+0, 0
$02D3	$3A03			XORLW	3
$02D4	$1903			BTFSC	STATUS, Z
$02D5	$2ABF			GOTO	L_Lcd_Custom_Out_27
$02D6	$0836			MOVF	FARG_Lcd_Custom_Out+0, 0
$02D7	$3A04			XORLW	4
$02D8	$1903			BTFSC	STATUS, Z
$02D9	$2AC2			GOTO	L_Lcd_Custom_Out_28
$02DA	$2AC5			GOTO	L_Lcd_Custom_Out_29
$02DB	$	L_Lcd_Custom_Out_24:
$02DB	$0337			DECF	FARG_Lcd_Custom_Out+1, 0
$02DC	$00F0			MOVWF	STACK_0
$02DD	$0836			MOVF	FARG_Lcd_Custom_Out+0, 0
$02DE	$07F0			ADDWF	STACK_0, 1
$02DF	$0870			MOVF	STACK_0, 0
$02E0	$00B6			MOVWF	FARG_Lcd_Custom_Out+0
$02E1	$0870			MOVF	STACK_0, 0
$02E2	$00C4			MOVWF	FARG_Lcd_Custom_Cmd+0
$02E3	$2130			CALL	_Lcd_Custom_Cmd
$02E4	$01A8			CLRF	Lcd_Custom_lcdcmd_status, 1
$02E5	$01B9			CLRF	Lcd_Custom_Out_i_L0, 1
$02E6	$	L_Lcd_Custom_Out_30:
$02E6	$0839			MOVF	Lcd_Custom_Out_i_L0, 0
$02E7	$0738			ADDWF	FARG_Lcd_Custom_Out+2, 0
$02E8	$0084			MOVWF	FSR
$02E9	$0800			MOVF	INDF, 0
$02EA	$00F0			MOVWF	STACK_0
$02EB	$0870			MOVF	STACK_0, 0
$02EC	$1903			BTFSC	STATUS, Z
$02ED	$2AF6			GOTO	L_Lcd_Custom_Out_31
$02EE	$0839			MOVF	Lcd_Custom_Out_i_L0, 0
$02EF	$0738			ADDWF	FARG_Lcd_Custom_Out+2, 0
$02F0	$0084			MOVWF	FSR
$02F1	$0800			MOVF	INDF, 0
$02F2	$00BF			MOVWF	FARG_Lcd_Custom_Chr_CP+0
$02F3	$2355			CALL	_Lcd_Custom_Chr_CP
$02F4	$0AB9			INCF	Lcd_Custom_Out_i_L0, 1
$02F5	$2AE6			GOTO	L_Lcd_Custom_Out_30
$02F6	$	L_Lcd_Custom_Out_31:
$02F6	$3001			MOVLW	1
$02F7	$00A8			MOVWF	Lcd_Custom_lcdcmd_status
$02F8	$0008			RETURN
$01A5	$	_main:
$01A5	$306E			MOVLW	110
$01A6	$1303			BCF	STATUS, RP1
$01A7	$1283			BCF	STATUS, RP0
$01A8	$00AA			MOVWF	lstr1_adc_example+0
$01A9	$3075			MOVLW	117
$01AA	$00AB			MOVWF	lstr1_adc_example+1
$01AB	$306C			MOVLW	108
$01AC	$00AC			MOVWF	lstr1_adc_example+2
$01AD	$306C			MOVLW	108
$01AE	$00AD			MOVWF	lstr1_adc_example+3
$01AF	$01AE			CLRF	lstr1_adc_example+4
;adc_example.c,9 :: 		void main ()
;adc_example.c,16 :: 		TRISA = 0x01;
$01B0	$3001			MOVLW	1
$01B1	$1683			BSF	STATUS, RP0
$01B2	$0085			MOVWF	TRISA
;adc_example.c,17 :: 		TRISB = 0x00;
$01B3	$0186			CLRF	TRISB, 1
;adc_example.c,18 :: 		TRISC = 0x00;
$01B4	$0187			CLRF	TRISC, 1
;adc_example.c,19 :: 		TRISD = 0x00;
$01B5	$0188			CLRF	TRISD, 1
;adc_example.c,20 :: 		TRISE = 0x00;
$01B6	$0189			CLRF	TRISE, 1
;adc_example.c,21 :: 		ADRESH = 0x00;
$01B7	$1283			BCF	STATUS, RP0
$01B8	$019E			CLRF	ADRESH, 1
;adc_example.c,22 :: 		ADRESL = 0x00;
$01B9	$1683			BSF	STATUS, RP0
$01BA	$019E			CLRF	ADRESL, 1
;adc_example.c,24 :: 		adcon0 = 0b10000001;
$01BB	$3081			MOVLW	129
$01BC	$1283			BCF	STATUS, RP0
$01BD	$009F			MOVWF	ADCON0
;adc_example.c,25 :: 		adcon1 = 0b11001110;
$01BE	$30CE			MOVLW	206
$01BF	$1683			BSF	STATUS, RP0
$01C0	$009F			MOVWF	ADCON1
;adc_example.c,27 :: 		for ( i=0; i<=5; i++)
$01C1	$1283			BCF	STATUS, RP0
$01C2	$01B5			CLRF	main_i_L0, 1
$01C3	$	L_main_0:
$01C3	$0835			MOVF	main_i_L0, 0
$01C4	$3C05			SUBLW	5
$01C5	$1C03			BTFSS	STATUS, C
$01C6	$29CF			GOTO	L_main_1
;adc_example.c,29 :: 		b[i]=0x00;
$01C7	$0835			MOVF	main_i_L0, 0
$01C8	$3F2F			ADDLW	main_b_L0
$01C9	$00F0			MOVWF	STACK_0
$01CA	$0870			MOVF	STACK_0, 0
$01CB	$0084			MOVWF	FSR
$01CC	$0180			CLRF	INDF, 1
;adc_example.c,30 :: 		}
$01CD	$	L_main_2:
;adc_example.c,27 :: 		for ( i=0; i<=5; i++)
$01CD	$0AB5			INCF	main_i_L0, 1
;adc_example.c,30 :: 		}
$01CE	$29C3			GOTO	L_main_0
$01CF	$	L_main_1:
;adc_example.c,33 :: 		lcd_custom_config(&PORTD,7,6,5,4,&PORTE,0,"null",1);
$01CF	$3008			MOVLW	PORTD
$01D0	$00B6			MOVWF	FARG_Lcd_Custom_Config+0
$01D1	$3007			MOVLW	7
$01D2	$00B7			MOVWF	FARG_Lcd_Custom_Config+1
$01D3	$3006			MOVLW	6
$01D4	$00B8			MOVWF	FARG_Lcd_Custom_Config+2
$01D5	$3005			MOVLW	5
$01D6	$00B9			MOVWF	FARG_Lcd_Custom_Config+3
$01D7	$3004			MOVLW	4
$01D8	$00BA			MOVWF	FARG_Lcd_Custom_Config+4
$01D9	$3009			MOVLW	PORTE
$01DA	$00BB			MOVWF	FARG_Lcd_Custom_Config+5
$01DB	$01BC			CLRF	FARG_Lcd_Custom_Config+6, 1
$01DC	$302A			MOVLW	lstr1_adc_example
$01DD	$00BD			MOVWF	FARG_Lcd_Custom_Config+7
$01DE	$3001			MOVLW	1
$01DF	$00BE			MOVWF	FARG_Lcd_Custom_Config+8
$01E0	$2004			CALL	_Lcd_Custom_Config
;adc_example.c,34 :: 		lcd_custom_cmd(0x0C);
$01E1	$300C			MOVLW	12
$01E2	$00C4			MOVWF	FARG_Lcd_Custom_Cmd+0
$01E3	$2130			CALL	_Lcd_Custom_Cmd
;adc_example.c,36 :: 		lcd_custom_cmd(0x01);
$01E4	$3001			MOVLW	1
$01E5	$00C4			MOVWF	FARG_Lcd_Custom_Cmd+0
$01E6	$2130			CALL	_Lcd_Custom_Cmd
;adc_example.c,37 :: 		delay_ms(2);
$01E7	$300D			MOVLW	13
$01E8	$00FB			MOVWF	STACK_11
$01E9	$30FF			MOVLW	255
$01EA	$00FA			MOVWF	STACK_10
$01EB	$0BFB			DECFSZ	STACK_11, F
$01EC	$29EE			GOTO	$+2
$01ED	$29F1			GOTO	$+4
$01EE	$0BFA			DECFSZ	STACK_10, F
$01EF	$29EE			GOTO	$-1
$01F0	$29EB			GOTO	$-5
$01F1	$30F3			MOVLW	243
$01F2	$00FA			MOVWF	STACK_10
$01F3	$0BFA			DECFSZ	STACK_10, F
$01F4	$29F3			GOTO	$-1
$01F5	$0000			NOP
;adc_example.c,38 :: 		while(1)
$01F6	$	L_main_3:
;adc_example.c,40 :: 		valor_adc = adc_read(0);
$01F6	$01B6			CLRF	FARG_Adc_Read+0, 1
$01F7	$22F9			CALL	_Adc_Read
;adc_example.c,42 :: 		IntToStr(valor_adc, b);
$01F8	$0870			MOVF	STACK_0, 0
$01F9	$1283			BCF	STATUS, RP0
$01FA	$00B6			MOVWF	FARG_IntToStr+0
$01FB	$0871			MOVF	STACK_0+1, 0
$01FC	$00B7			MOVWF	FARG_IntToStr+1
$01FD	$302F			MOVLW	main_b_L0
$01FE	$00B8			MOVWF	FARG_IntToStr+2
$01FF	$225E			CALL	_IntToStr
;adc_example.c,44 :: 		lcd_custom_out(2,2, b );
$0200	$3002			MOVLW	2
$0201	$00B6			MOVWF	FARG_Lcd_Custom_Out+0
$0202	$3002			MOVLW	2
$0203	$00B7			MOVWF	FARG_Lcd_Custom_Out+1
$0204	$302F			MOVLW	main_b_L0
$0205	$00B8			MOVWF	FARG_Lcd_Custom_Out+2
$0206	$22B6			CALL	_Lcd_Custom_Out
;adc_example.c,46 :: 		}
$0207	$29F6			GOTO	L_main_3
;adc_example.c,47 :: 		}
$0208	$2A08			GOTO	$
