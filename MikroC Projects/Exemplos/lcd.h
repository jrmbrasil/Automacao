...
#define _LCD_H
	...
	//rotina para mikroC - lcd 1602  com endereço linha 0 - 0x80 e linha 1 - 0xC0
	//
...
/*#ifdef	_HTC_H
	...
	void lcd_pos_xy(unsigned char, unsigned char);
	...
	void lcd_send_nibble(unsigned char dado)
	{
		unsigned char c;
		lcd_d4 =0;
		lcd_d5 =0;
	...
	void lcd_send_data(unsigned char valor)
	{
		RS = 1;
		lcd_send_nibble(valor>>4);
		lcd_send_nibble(valor & 0x0F);
	...
	void lcd_send_string(unsigned char *a)
	{
		int p=0;
		unsigned char b;
		b = a[p];
	...
	void	lcd_pos_xy(unsigned char x, unsigned char y)  //x coluna;  y linha
	{
...
char  ftoasc ( char * valor_string, float value, char precision)
{
	...
	itoa (number_string,  number, 10);  //modelo hitech
	itoa (frac_string, frac,  10);		//modelo hitech
